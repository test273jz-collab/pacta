...
export default function AdminUsers() {
  ...
  const toggleActive = async (user: User) => {
    ...
    try {
      ...
      if (data.success) {
        setUsers((prev) =>
          prev.map((u) =>
            u._id === user._id ? { ...u, isActive: data.data.isActive } : u,
          ),
        );
  ...
  const handleApproval = async (user: User, status: ApprovalStatus) => {
    ...
    try {
      ...
      if (data.success) {
        setUsers((prev) =>
          prev.map((u) =>
            u._id === user._id ? { ...u, approvalStatus: status } : u,
          ),
        );
        toast.success(
          status === "approved"
  ...
  const pendingCount = users.filter(
    (u) => PROVIDER_ROLES.includes(u.role) && u.approvalStatus === "pending",
  ).length;
    ...
    >
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">
              {filtered.length} {isRTL ? "مستخدم" : "users"}
            </p>
          </div>
        </div>
        {pendingCount > 0 && (
      ...
      <div className="bg-white rounded-3xl border border-pacta-sand p-5 mb-6 flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          <Filter
            size={13}
            className={`absolute top-1/2 -translate-y-1/2 text-gray-400 ${isRTL ? "right-4" : "left-4"} pointer-events-none`}
          />
          <select
            value={filterRole}
          ...
          <ChevronDown
            size={13}
            className={`absolute top-1/2 -translate-y-1/2 text-gray-400 ${isRTL ? "left-3" : "right-3"} pointer-events-none`}
          />
        </div>
        <div className="relative">
          <select
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-pacta-sand/40">
                {filtered.map((user) => {
                  ...
                  const isLoading = (k: string) =>
                    actionLoading === user._id + k;
                  return (
                    <tr
                      key={user._id}
                      className="hover:bg-pacta-sand/10 transition-colors"
                    >
                      {/* User */}
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-pacta-navy text-xs whitespace-nowrap">
                            {user.name}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        {ROLE_LABELS[user.role] && (
                          <span
                            className={`px-2.5 py-1 text-[10px] font-bold rounded-lg border uppercase ${ROLE_LABELS[user.role].color}`}
                          >
                            {isRTL
                      ...
                      <td className="p-4">
                        {isProvider ? (
                          <span
                            className={`flex items-center gap-1 w-fit px-2.5 py-1 text-[10px] font-bold rounded-lg border ${
                      ...
                      <td className="p-4">
                        <span
                          className={`px-2.5 py-1 text-[10px] font-bold rounded-lg border ${user.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-100 text-gray-500 border-gray-200"}`}
                        >
                          {user.isActive
                            ? isRTL
                              ? "نشط"
